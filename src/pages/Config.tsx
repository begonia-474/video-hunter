...
// ========== 字段工厂 ==========
// 所有可选 toggle 默认包含，通过 exclude 排除不需要的字段
function makeFields(
  ...
  const basics: FieldDef[] = [
    ...
    { key: "mode", label: "下载模式", type: "select", group: "basic", options: modeOptions },
  ];
...
const platformFields: Record<string, FieldDef[]> = {
  douyin: makeFields([
    { value: "one", label: "单个作品" }, { value: "post", label: "用户主页作品" },
    { value: "like", label: "点赞作品" }, { value: "collection", label: "收藏作品" },
    { value: "collects", label: "收藏夹" }, { value: "music", label: "收藏音乐" },
    { value: "mix", label: "合集作品" }, { value: "live", label: "直播录制" },
    { value: "feed", label: "推荐流" }, { value: "related", label: "相关推荐" },
    { value: "friend", label: "好友动态" },
  ]),
  tiktok: makeFields([
    { value: "one", label: "单个作品" }, { value: "post", label: "用户主页作品" },
    { value: "like", label: "点赞作品" }, { value: "collect", label: "收藏作品" },
    { value: "mix", label: "合集作品" }, { value: "search", label: "关键词搜索" },
    { value: "live", label: "直播录制" },
  ]),
  twitter: makeFields(
    [
      { value: "one", label: "单条推文" }, { value: "post", label: "用户推文" },
      { value: "like", label: "喜欢的推文" }, { value: "bookmark", label: "书签推文" },
    ],
    ["music", "cover", "desc"]
  ),
  weibo: makeFields(
    [
      { value: "one", label: "单条微博" }, { value: "post", label: "用户微博" },
    ],
    ["music", "cover", "desc"]
  ),
};
    ...
    <div className="flex flex-col h-full">
      <div className="px-6 pt-5 pb-3 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">管理各平台的下载配置</p>
        </div>
        ...
        <div className="w-44 shrink-0 border-r border-border px-2 py-2 overflow-auto">
          {platforms.map((p) => (
            ...
            >
              <span>{p.name}</span>
              {p.status !== "ready" && (
                <Badge variant={p.status === "developing" ? "warning" : "outline"} className="text-[10px] px-1 py-0">
          ...
          {Object.entries(grouped).map(([group, groupFields]) => {
            ...
            return (
              <div key={group} className="mb-6">
                {collapsible ? (
                  <button onClick={toggle} className="flex items-center gap-2 mb-3 text-sm font-medium text-muted-foreground hover:text-foreground transition-mac">
                    <svg width="12" height="12" viewBox="0 0 12 12" className={cn("transition-transform", expanded && "rotate-90")}>
                      <path d="M4 2l4 4-4 4" fill="none" stroke="currentColor" strokeWidth="1.5" />
                    </svg>
                    {groupLabels[group]}
                  </button>
                ) : (
                  <h3 className="text-sm font-medium text-muted-foreground mb-3">{groupLabels[group]}</h3>
                )}
                {expanded && (
                  <Card>
      ...
      <CookieDialog open={cookieDialogOpen} onClose={() => setCookieDialogOpen(false)} onSave={(cookie) => handleChange("cookie", cookie)} platform={activePlatform} />
    </div>
  );
}
...
function FieldRow({
    ...
    return (
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={cn("relative inline-flex h-5 w-9 items-center rounded-full transition-mac", value ? "bg-primary" : "bg-input")}
        >
          <span className={cn("inline-block h-3.5 w-3.5 rounded-full bg-white shadow-sm transition-transform", value ? "translate-x-4.5" : "translate-x-0.5")} />
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-1.5">
      ...
      {field.key === "cookie" ? (
        <div className="flex gap-2">
          <Input type="password" value={typeof value === "string" ? value : ""} onChange={(e) => onChange(field.key, e.target.value)} placeholder={field.placeholder} className="flex-1" />
          <Button variant="outline" size="sm" onClick={onOpenCookieDialog} className="shrink-0 gap-1.5">
            <Cookie size={14} /> 自动获取
          </Button>
        </div>
      ) : field.key === "path" ? (
        <div className="flex gap-2">
          <Input type="text" value={typeof value === "string" ? value : ""} onChange={(e) => onChange(field.key, e.target.value)} placeholder={field.placeholder} className="flex-1" />
          <Button
            variant="outline"
            size="sm"
            onClick={async () => {
        ...
        <Input type={field.type} value={typeof value === "string" ? value : ""} onChange={(e) => onChange(field.key, e.target.value)} placeholder={field.placeholder} />
      )}
      {field.description && <p className="text-xs text-muted-foreground">{field.description}</p>}
    </div>
  );
